...
class Analyzer
{
    ...
    private function getValue(string &$hand): int
    {
        ...
        uasort($comboList, static function ($a, $b) {
            if ($a['count'] === $b['count']) {
                return $a['value'] === $b['value'] ? 0 : ($a['value'] < $b['value'] ? 1 : -1);
            }
        ...
        reset($comboList);
        $firstSetKey = key($comboList);
        if (
            $comboList[$firstSetKey]['count'] < 5
            && isset($comboList[self::DENOM_VALUES['J']])
            && $comboList[self::DENOM_VALUES['J']]['count'] > 0
        ) {
            next($comboList);
            $secondSetKey = key($comboList);

            if ($firstSetKey === self::DENOM_VALUES['J']) {
                $comboList[$secondSetKey]['count'] += $comboList[self::DENOM_VALUES['J']]['count'];
                $comboList[self::DENOM_VALUES['J']]['count'] = 0;
            } else {
                $comboList[$firstSetKey]['count'] += $comboList[self::DENOM_VALUES['J']]['count'];
                $comboList[self::DENOM_VALUES['J']]['count'] = 0;
            }
        }
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            if (!isset($comboList[$i]) || $comboList[$i]['count'] === 0) {
                // printf("%2d => %16d = %-15s%-20s = %-d\n", 0, $type, decbin($type), decbin($value), $value);
                continue;
            }
